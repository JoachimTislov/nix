...
sudo nixos-generate-config --show-hardware-config > "$root/hosts/desktop/hardware-configuration.nix"
nixos-rebuild build --flake "$root#desktop"
read -r -p 'Install desktop to /mnt? [y/N] ' answer
[[ "$answer" =~ ^[Yy]$ ]] && sudo nixos-install --root /mnt --flake "$root#desktop"
